...
* Save and verify save failed
* Verify validation failed for "task[stage](text_field): Pipeline \"${runtime_name:upper_pipeline}\" tries to fetch artifact from stage \"${runtime_name:uppest_pipeline} :: uppest-stage3\" which does not complete before \"${runtime_name:upper_pipeline}\" pipeline's dependencies."
* Verify validation failed for "task[src](text_field): Should provide either srcdir or srcfile"
